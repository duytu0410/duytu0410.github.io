...
// Xử lí nút click
    btn[2].onclick=function(){
        ...
        var u = document.getElementById("username").value;
        $.ajax({
            async:false,//đồng bộ hóa hàm cho phần done chạy trước rồi mới chạy phía dưới
            method:"GET",
            url:"http://5dcc197385d1960014615be1.mockapi.io/user"
        }).done(function(data){
            var u = document.getElementById("username").value;
            var p = document.getElementById("password").value;
            a=data;
            for(i=0;i<a.length;i++){
                if(u==a[i].email&&p==a[i].password){
                    alert("Đăng nhập thành công");
                    m=i;
                    t=0;
                }
        ...
        if(t==0){
            ...
            $.get("http://5dcc197385d1960014615be1.mockapi.io/user", function(a){
                $(".dropdown").append('<i class="fas fa-user" style="padding-top:5px;padding-right:5px;"></i><div class="nick">'+a[m].name+'</div>')
            });
        }
    }
